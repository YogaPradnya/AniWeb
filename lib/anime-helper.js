 ...
 * Uses a batched parallel approach to avoid rate limits/timeouts.
 */
/**
 * Fetches all episodes by paginating through the upstream API.
 * Uses a batched parallel approach with error resilience.
 */
export async function getAllEpisodesUpstream(animeId) {
  try {
    ...
    if (!firstPage) return [];
    if (firstPage.length < 30) return firstPage;

    // 2. We will fetch up to 100 pages total (enough for ~3000 episodes)
    const maxPages = 100;
    const batchSize = 10;
    ...
    for (let i = 1; i < maxPages; i += batchSize) {
      ...
      console.log(`[Helper] Fetching episode batch: Pages ${batchIndices.join(', ')}`);
      
      // Fetch batch with individual error handling to prevent one failure from stopping the whole process
      const batchResults = await Promise.all(
        batchIndices.map(async (page) => {
          try {
            // Try twice for stability
      ...
      for (const result of batchResults) {
        ...
        if (Array.isArray(eps)) {
          ...
          // If we get an empty page or a page that's not full, it's the end
          if (eps.length < 30) {
            reachedEnd = true;
          }
        } else {
          // Individual page error - skip and continue to gather from other pages
          console.warn(`[Helper] Failed to fetch episode page ${result.page} for anime ${animeId}, skipping...`);
        }
      }
    }
    ...
    return Array.from(uniqueEpisodesMap.values()).sort((a, b) => {
