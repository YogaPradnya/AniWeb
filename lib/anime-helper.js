...
export async function getAllEpisodesUpstream(animeId) {
  try {
    // 1. Fetch first page to see if we need more
    const firstPage = await getEpisodeListUpstream(animeId, 0);
    if (!firstPage || firstPage.length < 30) return firstPage || [];

    // 2. If first page is full, fetch next pages in parallel
    // Big anime like One Piece can have ~40 pages. We fetch up to 50 pages.
    const pageIndices = Array.from({ length: 49 }, (_, i) => i + 1); // 1 to 49
    const results = await Promise.all(
      pageIndices.map(page => getEpisodeListUpstream(animeId, page))
    );
    ...
    for (const eps of results) {
      if (eps && eps.length > 0) {
        ...
        // Since it's paginated, once we hit an empty page, we could stop, 
        // but since we did parallel requests, we just continue to gather what we got.
      }
    }
    
    // Sort just in case (usually latest first from upstream)
    return allEpisodes;
  } catch (err) {
