...
export async function getGenres() {
  try {
    // Coba upstream dulu
    const res = await fetch('https://animeinweb.com/api/proxy/3/2/explore/genre', {
      headers: { ...HEADERS, 'Referer': 'https://animeinweb.com/' },
      cache: 'no-store'
    });
    if (res.ok) {
...
// ─── 12. Upstream Search ─────────────────────────────────────────────────────
// GET /api/proxy/3/2/explore/movie
export async function searchUpstream(keyword = '', opts = {}) {
  try {
    const params = new URLSearchParams();
    if (keyword) params.set('keyword', keyword);
    if (opts.genre) params.set('genre_in', opts.genre);
    if (opts.sort) params.set('sort', opts.sort); // upstream uses views, latest, alphabet, score, favorites
    if (opts.page !== undefined) params.set('page', opts.page);
    
    const url = `https://animeinweb.com/api/proxy/3/2/explore/movie?${params.toString()}`;
    console.log(`[Helper] searchUpstream: ${url}`);
    ...
    // Normalize response to match existing searchData format
    if (data.status === 200 && data.data) {
      return {
        success: true,
        data: wrapImageProxy(data.data),
        total: data.total || data.data.length,
        pagination: {
          currentPage: opts.page || 0,
          hasNextPage: data.data.length >= 20, // heuristic
    ...
    console.error('[Helper] searchUpstream failed:', err.message);
    return { success: false, data: [] };
  }
}
// ─── 13. Upstream Stream ─────────────────────────────────────────────────────
// GET /api/proxy/3/2/episode/streamnew/{episode_id}
export async function getStreamUpstreamByEpId(episodeId) {
  try {
    const url = `https://animeinweb.com/api/proxy/3/2/episode/streamnew/${episodeId}`;
    console.log(`[Helper] getStreamUpstreamByEpId: ${url}`);
    ...
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const data = await res.json();
    return (data.status === 200 && data.data) ? data.data : null;
  } catch (err) {
    console.error(`[Helper] getStreamUpstreamByEpId(${episodeId}) failed:`, err.message);
    return null;
  }
}

// ─── 14. Upstream Home Data ──────────────────────────────────────────────────
// GET /api/proxy/3/2/home/data
export async function getHomeUpstream() {
  try {
    const url = `https://animeinweb.com/api/proxy/3/2/home/data`;
    const res = await fetch(url, {
      headers: { ...HEADERS, 'Referer': 'https://animeinweb.com/' },
      cache: 'no-store'
    });
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const data = await res.json();
    return data.status === 200 ? wrapImageProxy(data.data) : null;
  } catch (err) {
    console.error('[Helper] getHomeUpstream failed:', err.message);
    return null;
  }
}

// ─── 15. Upstream Schedule ───────────────────────────────────────────────────
// GET /api/proxy/3/2/schedule/data?day=UPPERCASE
export async function getScheduleUpstream(day = 'SENIN') {
  try {
    const upperDay = day.toUpperCase();
    const url = `https://animeinweb.com/api/proxy/3/2/schedule/data?day=${upperDay}`;
    const res = await fetch(url, {
      headers: { ...HEADERS, 'Referer': 'https://animeinweb.com/' },
      cache: 'no-store'
    });
    ...
    console.error('[Helper] getScheduleUpstream failed:', err.message);
    return null;
  }
}

// ─── 16. Upstream Movie Detail ──────────────────────────────────────────────
// GET /api/proxy/3/2/movie/detail/{anime_id}
export async function getMovieDetailUpstream(animeId) {
  try {
    const url = `https://animeinweb.com/api/proxy/3/2/movie/detail/${animeId}`;
    const res = await fetch(url, {
      headers: { ...HEADERS, 'Referer': 'https://animeinweb.com/' },
      cache: 'no-store'
    });
    ...
    console.error(`[Helper] getMovieDetailUpstream(${animeId}) failed:`, err.message);
    return null;
  }
}

// ─── 17. Upstream Episode List ──────────────────────────────────────────────
// GET /api/proxy/3/2/movie/episode/{anime_id}
export async function getEpisodeListUpstream(animeId, page = 0) {
  try {
    const url = `https://animeinweb.com/api/proxy/3/2/movie/episode/${animeId}?page=${page}`;
    const res = await fetch(url, {
      headers: { ...HEADERS, 'Referer': 'https://animeinweb.com/' },
      next: { revalidate: 3600 } // Cache 1 Jam untuk respons API Upstream agar load super cepat
    });
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const data = await res.json();
    // Upstream returns { status, data: { episode: [...] } }
    return data.status === 200 && data.data?.episode ? wrapImageProxy(data.data.episode) : null;
  } catch (err) {
    console.error(`[Helper] getEpisodeListUpstream(${animeId}, page ${page}) failed:`, err.message);
    return null;
  }
}

// In-memory cache agar pindah-pindah episode tetap instant
const episodesCache = new Map();

/**
 * Fetches all episodes by paginating through the upstream API.
 * Uses a controlled batch approach with memory caching to prevent missing episodes from rate limits.
 */
export async function getAllEpisodesUpstream(animeId) {
  try {
    if (episodesCache.has(animeId)) {
      return episodesCache.get(animeId);
    }

    // 1. Fetch first page to see total/count
    const firstPage = await getEpisodeListUpstream(animeId, 0);
    if (!firstPage) return [];
    if (firstPage.length < 30) {
      episodesCache.set(animeId, firstPage);
      return firstPage;
    }

    // 2. Jika halamannya penuh (30 eps), tarik halaman berikutnya secara bertahap (batch)
    // Jangan menembak 100 request sekaligus karena server API pusat akan mem-blokir (Rate Limit) dan membuat episode bolong.
    let allEpisodes = [...firstPage];
    const maxPages = 100;
    const batchSize = 30;
    
    for (let i = 1; i < maxPages; i += batchSize) {
      const batchIndices = [];
      for (let j = i; j < i + batchSize && j < maxPages; j++) {
        batchIndices.push(j);
      }
      
      console.log(`[Helper] Fetching episode batch for ${animeId}: Pages ${batchIndices.join(', ')}`);
      
      const batchResults = await Promise.all(
        batchIndices.map(async (page) => {
          try {
            // Auto retry 1 kali jika gagal
            let res = await getEpisodeListUpstream(animeId, page);
            if (!res) res = await getEpisodeListUpstream(animeId, page); 
            return { page, data: res || [] }; // Pastikan selalu array walau gagal
          } catch (e) {
            return { page, data: [] };
          }
        })
      ...
      let gotAnyDataInBatch = false;
      let foundPageWithLessThan30 = false;
      
      for (const result of batchResults) {
        const eps = result.data;
        if (Array.isArray(eps) && eps.length > 0) {
          gotAnyDataInBatch = true;
          allEpisodes = allEpisodes.concat(eps);
          if (eps.length < 30) {
            foundPageWithLessThan30 = true;
          }
        }
      }
      
      // Jika SATUPUN halaman di batch ini tidak menghasilkan data (artinya kita benar-benar sudah di ujung dan bukan sekadar 1 halaman error)
      // ATAU jika kita menemukan halaman terakhir yang datanya ada tapi kurang dari 30 (ujung list)
      if (!gotAnyDataInBatch) {
        console.log(`[Helper] Batch empty, stopping fetch for ${animeId}`);
        break; 
      }
    }
    
    // Final de-duplication and sorting based on episode number
    const uniqueEpisodesMap = new Map();
    allEpisodes.forEach(ep => {
      const num = parseInt(ep.episodeNumber || ep.number || 0);
      if (num > 0) uniqueEpisodesMap.set(num, ep);
    });
    
    const finalEps = Array.from(uniqueEpisodesMap.values()).sort((a, b) => {
      const numA = parseInt(a.episodeNumber || a.number || 0);
      const numB = parseInt(b.episodeNumber || b.number || 0);
      return numB - numA; // Newest first
    });

    episodesCache.set(animeId, finalEps);
    return finalEps;
  } catch (err) {
    console.error(`[Helper] getAllEpisodesUpstream(${animeId}) failed:`, err.message);
    return [];
  }
}
