 ...
 * Uses a fully parallel approach with memory caching.
 */
export async function getAllEpisodesUpstream(animeId) {
  try {
    ...
    // 2. Fetch all other pages secara bersamaan (parallel) agar instan
    const maxPages = 100; // Approx 1200 episodes, cukup untuk One Piece
    let allEpisodes = [...firstPage];
    
    const pageIndices = Array.from({ length: maxPages - 1 }, (_, i) => i + 1);
    
    // Tembak semua page sekaligus, backend Next.js sanggup handle ratusan request parallel
