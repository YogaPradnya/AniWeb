...
export const animeApi = {
  ...
  getSchedule: async (day) => {
    if (typeof window === 'undefined') {
       const { getSchedule } = await import('./anime-helper');
       const result = await getSchedule(day);
       return result.data;
    }
    return fetch(`/api/v1/schedule?day=${day}`).then(r => r.json()).then(r => r.data);
  },
  
  getDetail: async (id) => {
    if (typeof window === 'undefined') {
       const { getAnimeDetail } = await import('./anime-helper');
       const result = await getAnimeDetail(id);
       return result.data;
    }
    return fetch(`/api/v1/detail?slug=${id}`).then(r => r.json()).then(r => r.data);
  },
