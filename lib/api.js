...
// lib/api.js
// Base URL dinamis dari .env (Server & Client side jika diperlukan)
const PRODUCTION_API = process.env.NEXT_PUBLIC_API_BASE_URL || process.env.API_BASE_URL || 'https://anime-api-three-jade.vercel.app/api/v1';
...
export const animeApi = {
  ...
  async _fetch(path, options = {}) {
    ...
    try {
      ...
      if (!fetchOpts.cache && !fetchOpts.next) {
  ...
  // Includes slider, hot, new, etc.
  getHome: async () => {
    try {
      const res = await fetch('/api/upstream/home/data');
  ...
  // === UPSTREAM STREAMING ===
  // Fetch direct from animeinweb.com via local proxy
  getStreamUpstream: async (episodeId) => {
    try {
      const res = await fetch(`/api/upstream/episode/streamnew/${episodeId}`);
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const json = await res.json();
      return json.success ? json.data : null;
    } catch (err) {
      console.error('[API] getStreamUpstream error:', err.message);
      return null;
    }
  },
};
